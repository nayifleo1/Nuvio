...
class CatalogService {
  ...
  async getHomeCatalogs(): Promise<CatalogContent[]> {
    ...
    try {
      // Get enabled catalogs in order of preference
      const enabledCatalogs = await stremioService.getEnabledCatalogs();
      
      for (const { addonId, addon, catalog } of enabledCatalogs) {
        try {
          // Get the items for this catalog
          const metas = await stremioService.getCatalog(addon, catalog.type, catalog.id, 1);
          ...
          console.error(`Failed to get catalog ${catalog.id} for addon ${addonId}:`, error);
        }
      }
    } catch (error) {
      console.error('Failed to get enabled catalogs:', error);
    }
  ...
  async getContentDetails(type: string, id: string): Promise<StreamingContent | null> {
    try {
      // Clean up the ID - sometimes it comes with season and episode numbers
      const idMatch = id.match(/tt\d+/);
      const cleanId = idMatch ? idMatch[0] : id;
      
      console.log(`Fetching content details for type: ${type}, original ID: ${id}, cleaned ID: ${cleanId}`);
      ...
      for (let i = 0; i < 3; i++) {
        try {
          meta = await stremioService.getMetaDetails(type, cleanId);
          if (meta) break;
          await new Promise(resolve => setTimeout(resolve, 1000 * Math.pow(2, i)));
        } catch (error) {
          lastError = error;
          console.error(`Attempt ${i + 1} failed to get content details for ${type}:${cleanId}:`, error);
          await new Promise(resolve => setTimeout(resolve, 1000 * Math.pow(2, i)));
        }
      }
      ...
      if (meta) {
        ...
        content.inLibrary = this.library[`${type}:${cleanId}`] !== undefined;
