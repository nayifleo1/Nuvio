...
import { TouchableOpacity, StyleSheet, Dimensions, Image, StatusBar, LayoutRectangle, LayoutChangeEvent, Platform } from 'react-native';
...
export function WhoIsWatching({ onProfileSelect }: Props) {
    ...
    const handleProfileSelect = useCallback(async (profile: Profile) => {
        // Debouncing: Prevent rapid-fire selections
        const now = Date.now();
        if (now - lastTouchTimestamp.current < 300) {
            return; // Ignore too-quick touches
        }
        lastTouchTimestamp.current = now;
        
        // Prevent multiple selection processes running at once
        if (isSelectionInProgress || isAnimating) {
            return;
        }
        ...
        // Immediately set selected state for visual feedback
        ...
        // Play sound after selection is already processing
        try {
        ...
        // Minimize and complete selection
        setTimeout(() => {
            setIsMinimizing(true);
            setTimeout(() => {
                runOnJS(onProfileSelect)(profile.id);
            }, 300); // Reduced the delay here
        }, 1600); // Slightly reduced from 2000
    }, [profileLayouts, isAnimating, isSelectionInProgress, spinnerRotation, onProfileSelect]);

    const handleProfileLayout = (profile: Profile, event: LayoutChangeEvent) => {
        if (Platform.OS === 'android') {
            // Simplified direct calculation for Android
            ...
            setProfileLayouts(prev => ({
                ...prev,
                [profile.id]: {
                    x: rect.x + (width - imageSize) / 2,
                    y: rect.y + (width - imageSize) / 2,
                    width: imageSize,
                    height: imageSize,
                    originalX: rect.x,
                    originalY: rect.y,
            ...
            // Original measure method for iOS
                            ...
                            <TouchableOpacity
                                ...
                                style={[
                                    ...
                                    Platform.OS === 'android' && { 
                                        ...
                                        paddingVertical: 10,
                                        backgroundColor: isAnimating && selectedId === profile.id ? 'rgba(255,255,255,0.1)' : 'transparent'
                                    }
                                ]}
                            ...
                            >
                                <Animated.View style={[
                                    ...
                                    <Animated.Image
                                        ...
                                        style={[
                                            styles.avatar,
                                            Platform.OS === 'android' && selectedId === profile.id && { 
                                                borderWidth: 2,
                                                borderColor: '#e50914'
                                            }
                                        ]}
